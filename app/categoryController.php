    ...
    class categoryController
    {
        public function get()
        {
            ...
            if($conn->connect_error==false){
                ...
                $query = "SELECT * FROM categoriesss";
            ...
            $conn = connect();
                if($conn->connect_error == false) {
                    ...
                    if($id !="" && $name !="" && $description !="" && $status!="" ){
                        ...
                        $prepared_query = $conn->prepare($query);
                        $prepared_query->bind_param('isss',$name,$description,$status);
                    if($prepared_query->execute()) {
                         header("Location:". $_SERVER['HTTP_REFERER'] );
                    }else{
                        header("Location:". $_SERVER['HTTP_REFERER'] );
        ...
        public fuction destroy($id)
        {
            ...
            if($conn->connect_error == false) {
                ...
                if($id !="" ){
                    ...
                    $prepared_query = $conn->prepare($query);
                    $prepared_query->bind_param('i',$name,$description,$status);
                    if($prepared_query->execute()) {
                        header("Location:". $_SERVER['HTTP_REFERER'] );
                   }else{
                       header("Location:". $_SERVER['HTTP_REFERER'] );
